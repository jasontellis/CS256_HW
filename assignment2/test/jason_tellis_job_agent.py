...
lookupTableFileName = "jason_tellis_hw2_lookuptable.dat"
...
acm = {"website": "acm",
       ...
       "descAttr": {"class": "aiResultsDescriptionNoAdvert"},
       }
...
def __preBuildLookUp__():
    ...
    preBuildQueries = [
        {"searchString": "computer science", "searchLoc": "San Jose"},
    ...
    for preBuildQuery in preBuildQueries:
        __fetchJobsIntoLookupTable__(preBuildQuery["searchString"], preBuildQuery["searchLoc"])
...
def main():
    ...
    searchLoc = args.loc
    __fetchJobsIntoLookupTable__(searchString, searchLoc)
    __writeToFile__(jobsLookupTable)


def __fetchJobsIntoLookupTable__(searchString="", searchLoc=""):
    """
    Fills job lookup table with a list of jobs from ACM, IEEE & Indeed for jobs matching given searchString & searchLoc
    ...
    for jobPage in jobPagesToSearch:
        try:
            jobs = __scrapeJobsFromPage__(jobPage, searchString, searchLoc)
            jobsLookupTable.extend(jobs)
            # print(len(jobsLookupTable))
        #
        except:
            print("No data found")
        __printJobs__()
...
def __scrapeJobsFromPage__(jobPage={}, searchString="", searchLoc=""):
    ...
    for jobRow in jobsHTMLTable:
        job = __extractJobFromRow__(jobRow, jobPage)
        searchVector = __getStringVector__(searchString)
        dist = __calculateDistance__(job["vector"], searchVector)
...
def __extractJobFromRow__(jobRow, jobPage):
    """
    ...
    jobDesc = __removeSpecialCharactersFromStr__(jobDesc)
...
def __getStringVector__(str=""):
    ...
    :param str: String containing words to be vectorized
    :return: Returns a bag of words vector with binary word counts
    """

    strippedJobDesc = __removeSpecialCharactersFromStr__(str)
    tempJobDescWordList = strippedJobDesc.lower().split(" ")
    jobDescWordList = __removeStopWords__(tempJobDescWordList)
    jobVector = __convertWordListToVector__(jobDescWordList)
...
def __readLookUpTableFromFile__(fileName=lookupTableFileName):
    ...
    if os.path.isfile(fileName):
        print("\nReading Lookup Table from Disk...")
        with open(fileName, "r") as fileHandle:
            tempLookupTable = pickle.load(fileHandle)
            jobsLookupTable.append(tempLookupTable)


def __writeToFile__(jobsLookupTable=[], fileName=lookupTableFileName):
...
def __findNearestNeighbours__(k=10):
