...
import datetime  # Used to calculate
...
# This needed deep understanding of Beautiful Soup to handle the inconsistent ways in which classes are labelled on the three websites
    ...
    def __findKJobs__(self):
        ...
        #If jobs not present in lookup table for search String, build lookup table from webpages and write it to disk
            ...
            lookupTable = Agent.__buildLookupTable__(self.searchString, self.searchLoc)
            if lookupTable:
                self.jobsLookupTable[self.searchString] = lookupTable
                Agent.__writeLookupTableToFile__(self.jobsLookupTable)
                neighbors = Agent.__findNearestNeighbours__(self.jobsLookupTable[self.searchString], self.k)
            else:
                print("No data found for "+self.searchString)
    ...
    def __preBuildLookUp__():
        ...
        if not jobsLookupTable:
            ...
            for preBuildQuery in preBuildQueries:
                jobsLookupTable[preBuildQuery["searchString"]] = Agent.__buildLookupTable__(preBuildQuery["searchString"])#,
            print("\nPre-building Completed. Lookup table with pages sorted by Jaccard Distance generated")
    ...
    def __buildLookupTable__(searchString="", searchLoc=""):
        ...
        #Sort jobs by distances from associated search string and store jobs hashed on search string
    ...
    def __extractJobFromRow__(jobRow, jobPage, searchString):
        ...
        jobDescVector = Agent.__getStringVector__(jobDesc+" "+jobTitle+" "+jobLoc)
        distance = 0.0
    ...
    def __convertWordListToVector__(wordList=[]):
        """
        Converts given list of words to a dictionary with words and their associated weighted frequency
