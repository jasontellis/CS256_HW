...
# This needed deep understanding of Beautiful Soup to handle the inconsistent ways in which classes are labelled on the three websites
    ...
    def __findKJobs__(self):
        ...
        # If look up table contains jobs for search string simply return k nearest jobs
        # Else build Lookup table for jobs

        neighbors = []
        if self.jobsLookupTable.has_key(self.searchString) and not self.jobsLookupTable[self.searchString]:
            ...
            neighbors = Agent.__findNearestNeighbours__(self.jobsLookupTable[self.searchString], 15)
